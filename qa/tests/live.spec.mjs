...
test('live customer catalog loads without ordering cues @live',async({page})=>{
  ...
  await expect(page.locator('#shippingPill')).toBeHidden();
  await expect(page.locator('#footerNote')).toBeHidden();
  await expect(page.locator('script[src*="catalog-pro-v9.js"]')).toHaveCount(1)
});

test('live product route is introduction-only @live',async({page})=>{
  ...
  await expect(page.getByText('ارسال رایگان',{exact:true})).toHaveCount(0);
