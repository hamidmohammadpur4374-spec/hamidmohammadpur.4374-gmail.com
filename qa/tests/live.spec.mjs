...
test('live customer catalog shows fixed store policy without ordering cues @live',async({page})=>{
  ...
  await expect(page.locator('script[src*="catalog-policy-v2.js"]')).toHaveCount(1);
  await expect(page.locator('script[src*="free-shipping-v1.js"]')).toHaveCount(0)
});

test('live product route is introduction-only with unified policy @live',async({page})=>{
  await waitForLive(page,'catalog/product.html?code=2233','h1.title');
  await expect(page.locator('#order,#copyOrder,#sticky,[data-order-ui]')).toHaveCount(0);
  ...
  await expect(page.getByText(/ثبت\s*سفارش|سفارش\s*محصول|کلیک\s*سفارش|نرخ\s*تبدیل/)).toHaveCount(0);
  await expect(page.locator('script[src*="product-pro-v8.js"]')).toHaveCount(1);
  await expect(page.locator('script[src*="catalog-policy-v2.js"]')).toHaveCount(1);
  await expect(page.locator('script[src*="product-intro-only-v1.js"]')).toHaveCount(0)
});

test('live admin login shell and hardened catalog policy runtime load @live',async({page})=>{
